...
// Guest fallback (brauzerda to'g'ridan-to'g'ri ochilganda test uchun)
function getGuestId() {
...
document.getElementById("coinCircle").addEventListener("click", async (e) => {
  ...
  } catch (err) { /* energy tugagan bo'lishi mumkin, jim o'tkazamiz */ }
});
...
document.getElementById("closeBtn").addEventListener("click", () => {
  if (tg) tg.close(); else showErr("Mini App yopildi (demo).");
});

// ===== Passive energy tick (visual only; server is source of truth) =====
setInterval(() => {
  if (!state) return;
...
async function init() {
  try {
    ...
    box.textContent = "Ulanishda xatolik: " + err.message + "\n\nBackend ishga tushganini va API_BASE manzili to'g'ri yozilganini tekshiring.";
  }
}
init();
